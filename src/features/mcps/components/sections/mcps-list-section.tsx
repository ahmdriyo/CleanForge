  ...
  return (
    <div className="space-y-4">
      {data.map((mcp) => (
        <div
          key={mcp.id}
          className="bg-white/75 backdrop-blur-xl border border-white/60 rounded-[20px] p-5 hover:bg-white/80 transition flex flex-col md:flex-row md:items-center gap-4"
        >
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <Badge
                className={`${mcp.status === "active" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-slate-100 text-slate-600"} rounded-full text-[11px]`}
                variant="outline"
              >
                {mcp.status}
              </Badge>
            ...
            <code className="block font-mono text-xs bg-slate-900 text-emerald-300 rounded-lg px-3 py-2 truncate">
              {mcp.endpoint}?token={mcp.token}
            </code>
            <div className="mt-2 text-xs text-slate-400">
              Usage: {mcp.usageCount} calls • Created{" "}
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Button
              ...
              onClick={() => handleCopy(mcp.endpoint, mcp.token, mcp.id)}
            >
              {copied === mcp.id ? (
            ...
            <Button
              ...
              onClick={() => toast("Regenerated token (dummy)")}
            >
            ...
            <Button
              ...
              size="sm"
              className="rounded-full"
              onClick={() => toast.success("Connection OK (dummy)")}
            >
              Test
            </Button>
          </div>
