...
const features = [
  {
    icon: FolderTree,
    title: "Visual Folder Tree Editor",
    desc: "Drag, edit, and visualize your structure. Every folder has rules, naming, and example code.",
    badge: "kebab-case",
  },
  {
    icon: MessageSquare,
    title: "AI Consultant",
    desc: "Chat with Gemini, get suggestions, click Apply to Standard to update your tree instantly.",
    badge: "Gemini",
  },
  {
    icon: Lock,
    title: "Private MCP Endpoint",
    desc: "Secure, isolated per-user MCP URL. Your standards stay private, powered by Firebase Auth.",
    badge: "Private",
  },
  {
    icon: Shield,
    title: "Isolated & Secure",
    desc: "Zero cross-user leakage. Firestore rules + Secret Manager for keys, deployed on Cloud Run.",
    badge: "Secure",
  },
];
      ...
      <div className="max-w-[1280px] mx-auto px-4 lg:px-8">
        ...
        >
          ...
          <h2 className="text-3xl font-semibold tracking-tight text-violet-950">
            Everything to Enforce Clean Code
          </h2>
        </motion.div>
          ...
          {features.map((f, i) => (
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-xs bg-violet-100 text-violet-700 border border-violet-200 rounded-full px-2.5 py-1">
                  {f.badge}
                </span>
              </div>
              <p className="text-sm text-slate-600 leading-relaxed">{f.desc}</p>
              {f.title.includes("Tree") && (
                <div className="mt-4 bg-slate-900 rounded-xl p-3 font-mono text-xs text-emerald-300">
                  src/features/auth → kebab-case ✓
                </div>
              )}
              {f.title.includes("MCP") && (
                <div className="mt-4 bg-violet-50 border border-violet-200 rounded-xl p-3 font-mono text-xs text-violet-900 truncate">
                  https://cleanforge.run.app/mcp/.../sse
                </div>
              )}
            </motion.div>
          ))}
